...
const ColorPaletteModal = ({ navigation }) => {
    ...
    const handelSubmit = useCallback(() => {
        if (!name) {
            Alert.alert('Please enter a palette name'); // not working on web browser testing.
    ...
    const handelChangeValue = useCallback((value, color) => {
        if (value === true) {
            setSelectedColors(colors => [...colors, color])
        } else {
            setSelectedColors(colors =>
                colors.filter(selectedColor => color.colorName !== selectedColor.colorName)
            )
        }
    }, [])
                ...
                renderItem={({ item }) => (
                    <View style={styles.color}>
                        ...
                        <Switch
                            ...
                            onValueChange={selected => { handelChangeValue(selected, item) }}
                        />
                    </View>
                )}
            />
